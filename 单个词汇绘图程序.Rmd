...
#绘图
  ...
  p <- ggplot(table_map, aes(x = X, y = Y, fill = as.factor(answer))) + geom_tile(show.legend = F) + scale_fill_manual(values = color_map) + theme_bw(base_size = 18) + coord_equal(ratio = 1) + xlim(0,16) + ylim(0,20)
  print(p)
...
请在 word = 后输入 "要分析的词汇" , 请一定要加英文双引号！！！！！！
在 color_number = 后输入颜色编号
在 language = 后输入 "cn" 或 "en"，请一定要加英文双引号！！！！！！
```{r}
single_plot(word = "heed", color_number = 120, language = "cn")
```
